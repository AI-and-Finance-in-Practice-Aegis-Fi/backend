...
서비스 명을 우선적으로 언급하고 월 구독료, 전체 시트와 시트 활용률을 활용하여 월 낭비 금액을 추산한 뒤 언급하며
시트 활용률, 월 낭비 금액과 고스트 계정 수를 이용하여 위험등급을 매긴 뒤 언급합니다.

■ 위험도 기준
기본적으로 활용률을 90%이상은 Low, 70~89%는 Medium, 50~69%는 High 50%미만은 Critical로 판단합니다.
고스트 계정률이 10%이하라면 기존 평가가 바뀌지 않으며, 10%~20%라면 한 단계 높은 위험등급으로 판단합니다. 20~30%라면 두 단계 높은 위험등급으로,
30%이상이라면 세단계 높은 위험등급으로 판단합니다.
...
1. 전년도 같은 달과 비교하여 시트 활용률과 고스트 계정수를 비교 및 전월 대비 시트활용률과 고스트 계정 수를 비교하여
현재 낭비가 되고 있는 상황을 좀 더 자세히 분석합니다.
2. 전년도 및 전월 비교 정보를 활용하고 시트 활용률과 고스트 계정 수, 월 낭비 금액을 토대로 위험등급을 판단합니다.
3. 낭비 발생 원인은 ai의 분석을 통해 추론하여 제시합니다. (실제 데이터가 충분하지 않기 때문에)
...
1. 기업별 맞춤형 위험도 기준 운영 프로세스
 기존의 획일적인 연도별, 월별로 구분하는게 아니라 각 회사의 프로세스 및 진행중인 프로젝트 특성을 반영하여 
위험도 기준을 변화하는 시스템을 구축하는 것으로 개선합니다.
또한 각 회사별 위험 판단 결과에 대한 피드백을 지속적으로 수집하여 위험도 기준과 평가 프로세스를 
지속적으로 개선하는 시스템을 구축하는 것을 권고합니다.
2. 부서별 예산 예측 및 사전 경고 체제
 사후적인 관리 체계 뿐만 아니라 전체적인 비용을 줄이기 위해서는 사전적으로 경고하는 체계를 갖추는 것도 중요합니다.
 회사에서 계획한 지출을 유도하고 초과 예산 지출을 방지할 수 있는 방법을 구축합니다. 즉, 현재 사용하는 추세를 기반으로
 얼마나 지출이 될 것인지 예측하는 시스템을 구축합니다.


■ 핵심 한 줄 요약
(의사결정자를 위한 한 문장 결론)
Aegis-Fi는 지출 이상 징후와 SaaS 낭비 비용을 조기에 확인하여 기업의 비용 관리 효율성을 높일 수 있습니다.
...
AI 기반의 통제 시스템을 갖추어 기업 지출 관리 체계를 고도화 할 수 있습니다.
...
■ 위험도 기준
기본적으로 활용률을 90%이상은 Low, 70~89%는 Medium, 50~69%는 High 50%미만은 Critical로 판단합니다.
고스트 계정률이 5%이하라면 기존 평가가 바뀌지 않으며, 5~10%라면 한 단계 높은 위험등급으로,
10%~20%라면 두 단계 높은 위험등급으로 판단합니다. 20~30%라면 세 단계 높은 위험등급으로,
30%이상이라면 활용률과 무관하게 Critical단계로 판단합니다.
또한 전년도 동월 대비, 전월 대비 시트활용률이 모두 올랐다면 한 단계 낮은 위험 등급으로 판단하고,
반대로 시트 활용률이 모두 떨어졌다면 한 단계 높은 위험 등급으로 판단합니다.
그 외에 시트 활용률 변화가 위험도에 미치는 영향은 없습니다.(회사를 normal한 회사로 가정하여 작성했기 때문에)
...
■ 핵심 한 줄 요약
(의사결정자를 위한 한 문장 결론)
Aegis-Fi를 도입한다면 SaaS낭비 비용을 줄이고, AI 기반의 통제 시스템을 갖추어 기업 지출 관리 체계를 고도화 할 수 있습니다.
...
전문적이고 간결하게 작성하되, 확인되지 않은 사실을 단정하지 마세요."""
    ...
    return response.choices[0].message.content.strip()
>>>>>>> da2dfeac4747dd1e74237e1e8e6335389fc0bc60
